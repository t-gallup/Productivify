...
export function FindSunday(displayDay) {
  const dayOfWeek = displayDay.getDay();
  const diffToSunday = dayOfWeek;
  const sunday = new Date(displayDay);
  sunday.setDate(displayDay.getDate() - diffToSunday);
  return sunday;
}
...
export function SetSaturday(sunday, setDisplaySaturday) {
  const saturday = new Date(sunday);
  saturday.setDate(sunday.getDate() + 6);
  setDisplaySaturday(saturday);
}
